...
def _best_event_for(events: list[dict], endpoint: str) -> Cell:
    ...
    best = matches[0]
    return Cell(
        status=best["status"], detail=best.get("detail", "")[:120], phase=best.get("phase", "")
    )


def _aggregate_cluster(reports: list[Path]) -> tuple[dict[str, list[dict]], set[str]]:
    ...
    for p in reports:
        try:
            ...
            print(f"warn: cluster report {p} not found — skipping", file=sys.stderr)
            continue
        for e in events:
...
def _services_passing(events: list[dict]) -> list[str]:
    ...
    return sorted({e["service"] for e in events if e.get("status") in ("PASS", "WARN")})
...
def _verdict(monolith: Cell, cluster_services: list[str], cluster_status: str) -> tuple[str, str]:
    ...
    if not mono_ok and cluster_ok:
        if monolith.status == "ABSENT":
            return "MISSING-IN-MONOLITH", f"covered by {', '.join(cluster_services)}; not routed"
        if monolith.status == "FAIL":
            return "BROKEN-IN-MONOLITH", f"route exists but fails ({monolith.detail[:60]})"
...
def render_text(rows: list[Row], monolith_name: str, cluster_label: str) -> str:
    lines = [f"# Gap report: {monolith_name} vs {cluster_label}", ""]
    summary = _summary(rows)
    lines.append(
        "Summary: "
        + ", ".join(f"{v}={summary.get(v, 0)}" for v in VERDICT_ORDER if summary.get(v, 0))
    )
    lines.append("")
    ...
    for r in rows_sorted:
        lines.append(
            f"{r.endpoint:38} {r.monolith.status:10} {r.cluster_status:10} {r.verdict:22} {r.notes}"
        )
    return "\n".join(lines) + "\n"
...
def render_markdown(
    ...
    cluster_label: str,
    monolith_url: str = "",
    cluster_reports: list[Path] | None = None,
) -> str:
    ...
    icon = {"PASS": "✅", "WARN": "⚠️", "FAIL": "❌", "SKIP": "⏭", "ABSENT": "—"}
    for r in rows_sorted:
        ...
        out.append(f"| `{r.endpoint}` | {m} | {c} | **{r.verdict}** | {r.notes} |")
    out.append("")
    out.append("Re-generate with:")
    out.append("```bash")
    out.append("python tests/openai-compat/probe.py --base-url http://llm.ht.local --name monolith")
    out.append(
        "python -m pytest tests/openai-compat -q --harness-report=tests/openai-compat/report.json"
    )
    out.append(
        "python tests/openai-compat/gap.py --monolith tests/openai-compat/report-monolith.json \\"
    )
    out.append(
        "  --cluster tests/openai-compat/report.json --format markdown -o k8s/MONOLITH-GAP.md"
    )
    out.append("```")
    return "\n".join(out) + "\n"


def render_gum(rows: list[Row], monolith_name: str, cluster_label: str) -> int:
    ...
    if broken:
        hi = ("196", "✖ BROKEN COVERAGE")
    elif missing:
        hi = ("214", "▲ COVERAGE GAPS")
    else:
        hi = ("42", "● FULL COVERAGE")
    ...
    for r in rows_sorted:
        ...
        cluster_str = ", ".join(r.cluster_services) if r.cluster_services else r.cluster_status
        subprocess.run(
            [
                "gum",
                "style",
                "--foreground",
                "252",
                f"    {r.endpoint:38} mono={r.monolith.status:6} cluster={cluster_str}",
            ],
            check=False,
        )
        if r.notes:
            subprocess.run(
                ["gum", "style", "--faint", "--foreground", "240", f"      {r.notes}"], check=False
            )
...
def _argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--monolith", required=True, help="path to a probe report (the monolith target)")
    p.add_argument(
        "--cluster",
        action="append",
        default=[],
    ...
    p.add_argument(
        "--monolith-url", default="", help="optional base URL for inclusion in the markdown header"
    )
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    monolith_name = monolith_events[0]["service"] if monolith_events else monolith_path.stem

    cluster_paths = [Path(c) for c in args.cluster] or [HERE / "report.json"]
    ...
    fmt = args.format or ("gum" if shutil.which("gum") and not args.output else "text")
    if fmt == "markdown":
        out = render_markdown(
            rows,
            monolith_name,
        ...
        if args.output:
            print("warning: --format gum ignores --output; use text/markdown", file=sys.stderr)
